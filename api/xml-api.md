...
**Пример ответа при внутренней ошибке:**

```xml
<root>
    <code>PROVIDER_ERROR | INTERNAL_ERROR</code>
    <message>Текст сообщения на русском языке (для пользователя)</message>
    <debug>Отладочная информация (можно писать в лог-файл; пользователю не показывается).</debug>
    <response>
        Тот же самый формат ответа из элемента response, что и в примере выше (с ограничением по service_key услуги).
        В большинстве случаев в ответе будут отсутствовать активационные данные для услуги, т.к. их получение
        с сервисов вендора происходит в асинхронном режиме и с непредсказуемыми задержками. См. описание
        экранов и блок-схему в другом документе за более подробной информацией.
    </response>
</root>
```
...
где по сравнению с предыдущим запросом добавляются следующие параметры:
* **ag_action**:  "activate" – подключить услугу, "deactivate" – отключить услугу.
* **ag_email**:  email пользователя. Его необходимо обязательно запросить у пользователя в форме подключения услуги. Важно, чтобы пользователь его ввел там именно сам и "с нуля", т.к. email должен быть настоящим: на него высылаются активационные данные (причем некоторые вендоры высылают их самостоятельно, минуя Velvica).
* **seller**: идентификатор менеджера, подключающего услугу. Необязательный параметр, его значение необходимо уточнить у Velvica.
Также параметр `ag_service_key` в данном запросе является обязательным: это ключ услуги, которую надо подключить или отключить (см. элемент `<service_key>` выше).

Пример ответа при подключении услуги

```xml
<root>
    <code>
        OK | USER_INVALID_EMAIL | USER_INVALID_SERVICE_KEY | USER_NO_MONEY | USER_UNKNOWN_UUID |
        USER_UNKNOWN_SELLER_ID | USER_INVALID_PERSONAL_CODE | USER_ALREADY_ACTIVATED |
        PROVIDER_ERROR | INTERNAL_ERROR | BAD_REQUEST
    </code>	
    <message>Текст сообщения на русском языке (для пользователя)</message>
    <debug>Отладочная информация (можно писать в лог-файл; пользователю не показывается).</debug>
    <response>
...
где **INTERNAL_ERROR** означает "Возникла непредвиденная ошибка на стороне Velvica", а PROVIDER_ERROR – "Непредвиденная ошибка на стороне провайдера" (техническое описание будет в этом случае в элементе `<debug>`).

Пример ответа при отключении услуги

```xml
<root>
    <code>
        OK | USER_INVALID_SERVICE_KEY | USER_UNKNOWN_UUID |  PROVIDER_ERROR | INTERNAL_ERROR
    </code>
    <message>Текст сообщения на русском языке (для пользователя)</message>
    <debug>Отладочная информация (можно писать в лог-файл; пользователю не показывается).</debug>
    <response>
